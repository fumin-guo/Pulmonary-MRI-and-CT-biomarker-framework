...
static __global__ void krnl_1(float *dvx, float *dvy, float *dvz,
        ...
        float *Ux, float *Uy, float *Uz,
        float cc, int iNx, int iNy, int iNz){
    ...
    if( ( (idx%iNy) != (iNy-1) ) &&
        ...
        float tx = dvx[idx] - ux[idx]/cc;
        float ty = dvy[idx] - uy[idx]/cc;
        float tz = dvz[idx] - uz[idx]/cc;
        
        q[idx] = (Gt[idx] - cc*(Gx[idx]*tx + Gy[idx]*ty + Gz[idx]*tz))/(1 + cc*Gf[idx]);
        q[idx] = MAX(MIN(q[idx],1),-1);     /*this was added by Fumin Guo, 2015/08/28*/  
...
static __global__ void krnl_7(float *bx1, float *bx2, float *bx3,
        ...
        float *Gx, float *Gy, float *Gz,
        float *q, float *ux, float *uy, float *uz, float *FPS,
        float cc, int iNx, int iNy, int iNz){
    ...
    if( ( (idx%iNy) != (iNy-1) ) &&
        ...
        /* update ux */
        float fpt = cc*(dvx[idx] + q[idx]*Gx[idx]);
        FPS[idx] = fabsf(fpt);
        ...
        fpt = cc*(dvy[idx] + q[idx]*Gy[idx]);
        uy[idx]  -= fpt;
        
        /* update uz */
        
        fpt = cc*(dvz[idx] + q[idx]*Gz[idx]);
        uz[idx] -= fpt;
